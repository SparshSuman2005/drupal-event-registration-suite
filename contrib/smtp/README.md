...
1.  Install this module using Composer. Doing so will also install the PHPMailer dependency.
    ...
    <pre><code>
    composer require phpmailer/phpmailer:6.6.3
    </code></pre>
...
Google Mail Authentication:
  ...
  Several details must be adjusted on the GMail account:
  * Manually log into the account through the website and accept all of the
    license agreements, etc.
  * The 'Less secure app access' option must be enabled on the account.
  * Multi/Two-factor authentication (2FA/TFA) must be disabled on the account.
  * An 'SMTP relay service' rule may be required in order for the account to
    allow email to be forwarded through the system; this must be created by the
  ...
  early 2021 and new accounts will require it from mid-2020. This restriction
  means that GMail and GSuite email addresses cannot be used without also
  enabling OAuth2, which is not currently supported by the module
...
General debugging
  ...
  If email is failing to deliver, enable the 'debug' option in the settings form
  and send a test email, that will show the responses from the server, which
  will provide insights into the reason for the failure.