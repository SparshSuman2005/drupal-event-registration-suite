...
class SMTPMailSystem implements MailInterface, ContainerFactoryPluginInterface {
  ...
  /**
   * SMTP Advanced Config Object.
   *
   * @var \Drupal\Core\Config\ImmutableConfig
   */
  protected $smtpAdvancedConfig;
   ...
   *   The session.
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, LoggerChannelInterface $logger, MessengerInterface $messenger, EmailValidatorInterface $emailValidator, ConfigFactoryInterface $config_factory, AccountProxyInterface $account, FileSystemInterface $file_system, MimeTypeGuesserInterface $mime_type_guesser, RendererInterface $renderer, SessionInterface $session) {
    $this->smtpConfig = $config_factory->get('smtp.settings');
    $this->smtpAdvancedConfig = $config_factory->get('smtp.advanced');
    $this->logger = $logger;
  ...
  public function mail(array $message) {
    if (!class_exists(PHPMailer::class)) {
      $this->logger->error($this->t('Unable to send mail: PHPMailer class was not found.'));
      return FALSE;
    }
    ...
    // Turn on debugging, if log level is DEBUG.
    $log_level = $this->smtpConfig->get('log_level') ?? RfcLogLevel::ERROR;
    if ($log_level === RfcLogLevel::DEBUG) {
      ...
      $mailer->Debugoutput = function ($message, $debug_level) {
        $sanitized_message = $this->sanitizeDebugMessage($message);
    ...
    foreach ($headers as $key => $value) {
      ...
      switch ($key) {
          ...
          switch ($vars[0]) {
            ...
            default:
              ...
              $this->messenger->addMessage($this->t('The %header of your message is not supported by PHPMailer and will be sent as text/plain instead.',
                ...
                'error');
              $this->logger->error($this->t('The %header of your message is not supported by PHPMailer and will be sent as text/plain instead.',
                ['%header' => "Content-Type: $value"]));
        ...
        case 'reply-to':
          ...
          if (!empty($reply_to_email) && $reply_to_email !== $from) {
    ...
    if ($this->smtpAdvancedConfig?->get('enabled')) {
      $mailer->SMTPOptions = $this->getAdvancedOptions();
    }
    ...
    return $this->smtpMailerSend($variables);
  }
   ...
   */
  public function smtpMailerSend(array $variables) {
    ...
    $this->logger->info('Sending mail to: @to (subject: %subject)', ['@to' => $to, '%subject' => $mailer->Subject]);
    ...
    try {
      ...
      $this->logger->error('Error sending e-mail from @from to @to: @error_message', [
        '@from' => $from,
        '@to' => $to,
  ...
  /**
   * Sanitize debug messages to remove sensitive information.
   *
   * @param string $message
   *   The debug message to sanitize.
   *
   * @return string
   *   The sanitized debug message.
   */
  protected function sanitizeDebugMessage($message) {
    // Get the username and password from config for more accurate masking.
    $username = $this->smtpConfig->get('smtp_username');
    $password = $this->smtpConfig->get('smtp_password');

    // Mask AUTH PLAIN commands (base64-encoded username:password).
    // Pattern: AUTH PLAIN followed by base64 string.
    $message = preg_replace(
      '/AUTH PLAIN\s+[A-Za-z0-9+\/=]+/',
      'AUTH PLAIN [REDACTED]',
      $message
    );

    // Mask any base64 strings that might be credentials (common in AUTH).
    // This catches AUTH LOGIN and other authentication methods.
    $message = preg_replace_callback(
      '/AUTH\s+\w+\s+([A-Za-z0-9+\/=]{20,})/',
      function ($matches) {
        // Try to decode and check if it contains password-like patterns.
        $decoded = base64_decode($matches[1], TRUE);
        if ($decoded !== FALSE && (strpos($decoded, ':') !== FALSE || strlen($decoded) > 10)) {
          return preg_replace('/AUTH\s+\w+\s+[A-Za-z0-9+\/=]+/', 'AUTH [METHOD] [REDACTED]', $matches[0]);
        }
        return $matches[0];
      },
      $message
    ...
    // Mask common password patterns (always, even if password not configured).
    $message = preg_replace('/password[:\s=]+([^\s\n\r]+)/i', 'password: [REDACTED]', $message);
    $message = preg_replace('/pass[:\s=]+([^\s\n\r]+)/i', 'pass: [REDACTED]', $message);

    // Mask password if it appears in plain text.
    if (!empty($password)) {
      $message = str_replace($password, '[REDACTED]', $message);
    }

    // Mask username if it appears (less sensitive but still good practice).
    if (!empty($username)) {
      // Only mask if it's not an email address pattern in a command context.
      $message = preg_replace('/\b' . preg_quote($username, '/') . '\b/', '[REDACTED]', $message);
    }

    // Mask common SMTP authentication patterns.
    $message = preg_replace('/334\s+([A-Za-z0-9+\/=]+)/', '334 [REDACTED]', $message);
  ...
  public function debug() {
    ...
    $debug_logs = $this->session->get('smtp_debug', []);
    $log_level = $this->smtpConfig->get('log_level') ?? RfcLogLevel::ERROR;
    if ($log_level === RfcLogLevel::DEBUG && !empty($debug_logs)) {
      $item_list = [
        '#theme' => 'item_list',
        '#items' => [],
      ];
      foreach ($debug_logs as $debug_log) {
        // Sanitize again as a safety net (in case sanitization wasn't applied).
        $sanitized_message = $this->sanitizeDebugMessage($debug_log['message']);
        $item_list['#items'][] = $sanitized_message;
      }
  ...
  protected function getAdvancedOptions() {
    $values = $this->smtpAdvancedConfig->get();
    $options = [];
